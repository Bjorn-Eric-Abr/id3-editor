...
async function main() {
  ...
  if (!targetPath) {
    ...
    console.error(
      chalk.red("Please provide a file or directory path as an argument."),
    );
...
export async function handleSingleFile(filePath: string) {
  ...
  if (initialGenre && !commonGenres.includes(initialGenre)) {
    ...
    initialGenreIndex = genreChoices.findIndex(
      (choice) => choice.name === initialGenre,
    );
  }
  ...
  try {
    const prompt = createFormPrompt(
      ...
      [
        {
          name: "artist",
          message: chalk.blue("Artist"),
          initial: tags.artist || "",
        },
        {
          name: "title",
          message: chalk.blue("Title"),
          initial: tags.title || "",
        },
        {
          name: "album",
          message: chalk.blue("Album"),
          initial: tags.album || "",
        },
...
export async function handleDirectory(dirPath: string) {
  ...
  const mp3Files = files
    .filter((f: string) => path.extname(f).toLowerCase() === ".mp3")
  ...
  logBox(
    "Batch Mode:",
    `Found ${mp3Files.length} MP3 files in ${path.basename(dirPath)}`,
    chalk.magenta,
  );
  ...
  try {
    ...
    console.log(
      chalk.green(
        `✔ Updated tags for ${successCount}/${mp3Files.length} files.\n`,
      ),
    );
    ...
    if (rename) {
      ...
      for (const file of mp3Files) {
        ...
        if (newName !== originalName) {
          ...
          try {
            ...
            console.warn(
              chalk.yellow(`⚠ Failed to rename ${originalName}: ${err}`),
            );
          }
        }
      }
      ...
      console.log(
        chalk.green(`✔ Renamed ${renameCount}/${mp3Files.length} files.`),
      );
    }
  } catch (err) {
    console.log(chalk.red("\nExiting without edits."));
